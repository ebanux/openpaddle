...
import { BaseAfterPaymentPageProps, AppointmentPreparationInfoAPData, AppointmentBookingPageLinkConfig } from '../../types';
import Card from '../../components/common/Card';
import Button from '../../components/common/Button';

const AppointmentPreparationInfoDisplay: React.FC<BaseAfterPaymentPageProps<AppointmentPreparationInfoAPData>> = ({ sessionData, templateData, theme, environment }) => {
  const cardTitle = templateData.title || "Prepare for Your Appointment";
  const pageConfig = sessionData.pageConfig as AppointmentBookingPageLinkConfig;
  const serviceName = pageConfig?.serviceTitle || "Your Appointment";
  ...
  return (
    <div className={`min-h-screen ${theme.bgClass} ${theme.textClass} py-8 px-4 flex flex-col items-center`}>
      <Card className={`${theme.cardBgClass} p-6 sm:p-8 shadow-2xl max-w-lg w-full`}>
        ...
        <h2 className={`text-xl font-semibold mb-3 text-center ${accentColorClass}`}>{templateData.preparationHeadline} for {serviceName}</h2>
        ...
        {templateData.preparationSteps && templateData.preparationSteps.length > 0 && (
          <div className={`p-4 rounded-md mb-4 ${theme.inputBgClass || 'bg-slate-50'} border ${theme.inputBorderClass || 'border-slate-200'}`}>
            <h3 className="text-md font-semibold opacity-90 mb-2">Please do the following:</h3>
        ...
        {templateData.whatNotToDo && templateData.whatNotToDo.length > 0 && (
          <div className={`p-4 rounded-md mb-4 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-700`}>
            <h3 className="text-md font-semibold text-red-700 dark:text-red-300 mb-2">Please AVOID:</h3>
        ...
        {templateData.locationMapEmbedUrl && (
          <div className="my-6">
            <h3 className="text-md font-semibold opacity-90 mb-2">Location:</h3>
            <div className="aspect-w-16 aspect-h-9 rounded-lg overflow-hidden border border-slate-300">
              <iframe
                src={templateData.locationMapEmbedUrl}
                width="100%"
                height="100%"
                style={{ border: 0 }}
        ...
        <p className="text-xs opacity-60 text-center mt-8">
          Booking ID: {sessionData.sessionId} (Mode: {environment.toUpperCase()})
        </p>
      </Card>
    </div>
  );
...
export default AppointmentPreparationInfoDisplay;