...
import { BaseAfterPaymentPageProps, DigitalContentSupportMoreAPData } from '../../types';
import Card from '../../components/common/Card';
import Button from '../../components/common/Button';

const DigitalContentSupportMoreDisplay: React.FC<BaseAfterPaymentPageProps<DigitalContentSupportMoreAPData>> = ({ sessionData, templateData, theme, environment }) => {
  const cardTitle = templateData.title || "Enjoy Your Content & Support Us!";
  ...
  return (
    <div className={`min-h-screen ${theme.bgClass} ${theme.textClass} py-8 px-4 flex flex-col items-center`}>
      <Card className={`${theme.cardBgClass} p-6 sm:p-8 shadow-2xl max-w-lg w-full`}>
        ...
        {templateData.supportHeadline && (
            <h2 className={`text-xl font-semibold mb-3 text-center ${accentColorClass}`}>{templateData.supportHeadline}</h2>
        )}
        {templateData.supportMessage && (
            <p className="text-sm opacity-80 text-center mb-6">{templateData.supportMessage}</p>
        )}

        <div className="space-y-3 my-6">
            {templateData.viewOtherProductsLink && (
                 <Button fullWidth onClick={() => window.open(templateData.viewOtherProductsLink, '_blank')} className={theme.buttonPrimaryClass}>
                    {templateData.viewOtherProductsButtonText || "View More Products"}
                </Button>
            )}
            {templateData.merchandiseLink && (
                <Button fullWidth onClick={() => window.open(templateData.merchandiseLink, '_blank')} className={theme.buttonSecondaryClass}>
                    {templateData.merchandiseButtonText || "Shop Our Merch"}
                </Button>
            )}
        </div>
        ...
        {templateData.socialMediaLinks && templateData.socialMediaLinks.length > 0 && (
            <div className={`my-6 pt-4 border-t ${theme.inputBorderClass || 'border-slate-300'} border-dashed`}>
                {templateData.socialMediaPrompt && <h3 className="font-medium opacity-90 text-center mb-3">{templateData.socialMediaPrompt}</h3>}
                    ...
                    {templateData.socialMediaLinks.map((social, index) => (
                        <a key={index} href={social.link} target="_blank" rel="noopener noreferrer" title={social.platform}
                           ...
                           {social.iconSvg ? (
                               ...
                               social.platform.substring(0,1) // Fallback to first letter
                           )}
                        </a>
                    ))}
                </div>
...
export default DigitalContentSupportMoreDisplay;
