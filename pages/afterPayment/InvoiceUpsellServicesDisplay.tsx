...
import { BaseAfterPaymentPageProps, InvoiceUpsellServicesAPData } from '../../types';
import Card from '../../components/common/Card';
import Button from '../../components/common/Button';

const InvoiceUpsellServicesDisplay: React.FC<BaseAfterPaymentPageProps<InvoiceUpsellServicesAPData>> = ({ sessionData, templateData, theme, environment }) => {
      ...
      <Card className={`${theme.cardBgClass} p-6 sm:p-8 shadow-2xl max-w-lg w-full`}>
        ...
        {templateData.upsellItems && templateData.upsellItems.length > 0 && (
          <div className="mt-8 pt-6 border-t border-dashed border-slate-300">
            <h3 className={`text-lg font-semibold mb-4 text-center ${accentColorClass}`}>{templateData.upsellSectionTitle || "Explore Our Other Services"}</h3>
                ...
                <Card key={index} className={`${theme.inputBgClass || 'bg-slate-50'} p-3 border ${theme.inputBorderClass || 'border-slate-200'} shadow-sm`}>
                    <div className="flex justify-between items-center">
                        <div>
                         ...
                         <a href={item.link} target="_blank" rel="noopener noreferrer" className={`px-3 py-1 text-xs rounded-md ${theme.buttonSecondaryClass} ml-2 flex-shrink-0`}>
                            Learn More
                        </a>
                    </div>
                </Card>
...
export default InvoiceUpsellServicesDisplay;
